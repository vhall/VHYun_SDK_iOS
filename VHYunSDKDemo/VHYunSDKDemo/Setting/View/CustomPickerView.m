...
{
    if ([_dataSource respondsToSelector:@selector(pickerView:numberOfRows:)]) {
        return [_dataSource pickerView:self numberOfRows:component];
    }
    return 0;
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    if ([_dataSource respondsToSelector:@selector(pickerView:titleWithRow:)]) {
        mycom.text = [_dataSource pickerView:self titleWithRow:row];
    }
